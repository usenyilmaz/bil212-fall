...
public class ProbeHashMap<K, V> extends AbstractHashMap<K, V> {
  ...
  private int probeCount; // number of probes in last operation
  ...
  private int findSlot(int h, K k) {
    ...
    int j; // index while scanning table
    int i = 0; // probe iteration count
    do {
      j = (h + i * i) % capacity; // quadratic probing: h + i^2
      if (isAvailable(j)) { // may be either empty or defunct
        if (avail == -1)
          avail = j; // this is the first available slot!
        if (table[j] == null) {
          probeCount = i + 1; // record probe count
      ...
      i++; // next probe
    } while (i != capacity); // stop after probing at most capacity times
    probeCount = i + 1; // record final probe count
  ...
  /**
   * Returns the number of probes performed in the last operation.
   * 
   * @return the probe count from the last findSlot call
   */
  ...
  public boolean containsKey(K key) {
    // Trigger a normal lookup so that probeCount is set by findSlot
    V val = get(key);
    if (val != null)
      return true; // found and non-null value

    // If get returned null, the key may still exist with a null value.
    // Check table directly for the key, but do NOT modify probeCount (we rely on
    // the probeCount set by get).
    for (int h = 0; h < capacity; h++) {
      if (!isAvailable(h) && table[h].getKey().equals(key)) {
        return true;
      }
    }
    return false;
