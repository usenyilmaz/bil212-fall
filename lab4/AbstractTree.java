...
public abstract class AbstractTree<E> implements Tree<E> {
  ...
  /**
   * Returns the number of descendants of Position p (all nodes in the
   * subtree rooted at p excluding p itself).
   * Implemented recursively.
   *
   * @param p A valid Position within the tree
   * @throws IllegalArgumentException if p is not a valid Position for this tree.
   */
  public int countDescendants(Position<E> p) throws IllegalArgumentException {
    int count = 0;
    for (Position<E> c : children(p)) {
      // each child contributes 1 (the child itself) plus its own descendants
