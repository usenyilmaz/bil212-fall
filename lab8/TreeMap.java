...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  public int countLessThan(K k){
    checkKey(k);
    return countLessThan(root(), k);
  }

  private int countLessThan(Position<Entry<K, V>> p, K key) {
    if (isExternal(p)) {
      return 0;
    }
    int cmp = compare(p.getElement().getKey(), key);
    if (cmp > 0) {
      return countLessThan(left(p), key);
    } else {
      return subtreeSize(left(p)) + 1 + countLessThan(right(p), key);
    }
  }
  private int subtreeSize(Position<Entry<K, V>> p) {
    if (isExternal(p)) {
      return 0;
    }
  ...
  public K inOrderPredecessor(K key) {
